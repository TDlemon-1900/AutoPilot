...
// 定义全局变量
int VCU_DrivingMode,Brake_Switch,Parking_StatusFeedback,Gear_Feedback,Door_Status,Gear_OperationStatus,
Parking_OperationStatus,VCU_FaultLevel,Brake_ControlMode,Manual_TakeoverStatus,EPB_Status;

float Actual_Break_Pre,Max_V_Speed,SteWheel_Torque,SteWheel_Angle,Average_Speed,Acceleration,Motor_TorqueFb,
Max_Brake_Deceleration,Pedal_Per;
...
int main(int argc, char **argv)
{
	...
	while (running) {
		...
		for (i = 0; i < num_events; i++) {  /* check waiting CAN RAW sockets */
			...
			// 0x61A no error!
			if(frame.can_id == 0x0cffa04a)
			{	
				VCU_DrivingMode = frame.data[0] & 0x03;
				Brake_Switch = (frame.data[0] & 0x40) >> 6;
				Parking_StatusFeedback=(frame.data[0] & 0x80) >> 7;
				Gear_Feedback = frame.data[1] & 0x03;
				Door_Status = (frame.data[1] & 0x04) >> 2;
				Gear_OperationStatus = (frame.data[1] & 0x08) >> 3;
				Parking_OperationStatus = (frame.data[1] & 0x10) >> 4;
				VCU_FaultLevel = (frame.data[1] & 0x60) >> 5;
				Brake_ControlMode = (frame.data[1] & 0x80) >> 7;
				Manual_TakeoverStatus = (frame.data[2] & 0x10) >> 4;
				EPB_Status = (frame.data[2] & 0x20) >> 5;
				Actual_Break_Pre = frame.data[3] * 0.1;
				Max_V_Speed = frame.data[4];
				SteWheel_Torque = frame.data[5] * 0.1 + 12.5;
				SteWheel_Angle = (frame.data[7] * 256 | frame.data[6] ) * 0.1 - 870;

				std::cout<<"VCU驾驶模式="<<VCU_DrivingMode<<std::endl;
				std::cout<<"制动开关="<<Brake_Switch<<std::endl;
				std::cout<<"驻车状态反馈="<<Parking_StatusFeedback<<std::endl;
				std::cout<<"档位反馈="<<Gear_Feedback<<std::endl;
				std::cout<<"车门状态="<<Door_Status<<std::endl;
				std::cout<<"档位操作状态="<<Gear_OperationStatus<<std::endl;
				std::cout<<"驻车操作状态="<<Parking_OperationStatus<<std::endl;
				std::cout<<"VCU故障等级="<<VCU_FaultLevel<<std::endl;
				std::cout<<"制动控制方式="<<Brake_ControlMode<<std::endl;
				std::cout<<"人工接管状态="<<Manual_TakeoverStatus<<std::endl;
				std::cout<<"EPB状态="<<EPB_Status<<std::endl;
				std::cout<<"实际主缸压力="<<Actual_Break_Pre<<std::endl;
				std::cout<<"最高车速="<<Max_V_Speed<<std::endl;
				std::cout<<"方向盘转矩信号="<<SteWheel_Torque<<std::endl;
				std::cout<<"方向盘转角="<<SteWheel_Angle<<std::endl;
			}
			
			if(frame.can_id == 0x0cffa04a)
			{
				Average_Speed = (frame.data[1] * 256 | frame.data[0] ) / 256;
				Acceleration = (frame.data[3] * 256 | frame.data[2] ) * 0.01 - 20.5;
				Motor_TorqueFb = (frame.data[5] * 256 | frame.data[4] ) * 0.1 - 3200;
				Max_Brake_Deceleration = frame.data[6] * 0.1;
				Pedal_Per = frame.data[7] - 100;

				std::cout<<"前轴平均速度="<<Average_Speed<<std::endl;
				std::cout<<"纵向加速度="<<Acceleration<<std::endl;
				std::cout<<"电机反馈扭矩="<<Motor_TorqueFb<<std::endl;
				std::cout<<"最大制动减速度="<<Max_Brake_Deceleration<<std::endl;
				std::cout<<"踏板百分比="<<Pedal_Per<<std::endl;
			}
